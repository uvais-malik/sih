...
const ProfileHeader: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center text-center">
            ...
            <h1 className="display-large text-3xl">{t('profile_greeting')}</h1>
            <p className="body-large text-gray-500 dark:text-gray-400 mt-1">{t('profile_location')}</p>
            <div className="mt-6 grid grid-cols-3 gap-4 w-full max-w-sm">
                <div className="text-center">
                    <p className="font-bold text-xl dark:text-white">{t('user_land_value')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('land_stat')}</p>
                </div>
                <div className="text-center">
                    <p className="font-bold text-xl dark:text-white">{t('user_crops_value')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('crops_stat')}</p>
                </div>
                <div className="text-center">
                    <p className="font-bold text-xl dark:text-white">{t('user_experience_value')}</p>
...
const Toggle: React.FC<{label: string; enabled: boolean; onChange?: () => void;}> = ({label, enabled, onChange}) => (
     <div className={`flex justify-between items-center ${onChange ? 'sm:col-span-2' : ''}`} onClick={onChange}>
        <p className="font-semibold text-gray-800 dark:text-gray-200">{label}</p>
        <div className={`w-14 h-8 rounded-full p-1 cursor-pointer transition-colors ${enabled ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-600'}`}>
    ...
    return (
        <div className="p-4 sm:p-6 space-y-6">
            ...
            <InfoCard title={t('personal_info')} icon="👤">
                <InfoField label={t('name_label')} value={t('user_name')} />
                <InfoField label={t('phone_label')} value={t('user_phone')} />
                <InfoField label={t('language_label')} value={t('user_language')} />
                <InfoField label={t('location_label')} value={t('user_location')} />
            </InfoCard>
             <InfoCard title={t('farm_info')} icon="🌾">
                <InfoField label={t('land_size_label')} value={t('user_land_size')} />
                <InfoField label={t('primary_crops_label')} value={t('user_primary_crops')} />
                <InfoField label={t('soil_type_label')} value={t('user_soil_type')} />
                <InfoField label={t('irrigation_label')} value={t('user_irrigation')} />
            </InfoCard>
